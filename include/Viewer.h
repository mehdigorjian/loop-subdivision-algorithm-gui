// This code was borrowed from libigl
#ifndef VIEWER_H
#define VIEWER_H
#include <Eigen/Core>
#include <cassert>
...
public:
    Viewer() : show_lines(true),
               ...
               callback_key_pressed([](igl::opengl::glfw::Viewer &viewer, unsigned int key, int modifiers) -> bool
                                    { return false; })
    {
  ...
  U,u      Show/hide texture
        ...
        const Eigen::MatrixXi &F)
    {
        ...
        if (UV.rows() != 0)
            FUV.resize(F.rows() * 3, UV.cols());
        if (NV.rows() != 0)
            FNV.resize(F.rows() * 3, NV.cols());
        ...
        for (int f = 0; f < F.rows(); f++)
        {
            for (int c = 0; c < 3; c++)
            {
                ...
                if (UV.rows() != 0)
                    FUV.row(FF(f, c)) = UV.row(F(f, c));
                if (NV.rows() != 0)
                    FNV.row(FF(f, c)) = NV.row(F(f, c));
            }
        }
        Eigen::MatrixXi T(FF.rows() * 1, 3);
        T << FF.col(0), FF.col(1), FF.col(2);
        viewer.data().clear();
        viewer.data().set_mesh(FV, T);
        if (NV.rows() != 0)
            viewer.data().set_normals(FNV);
        if (UV.rows() != 0)
            viewer.data().set_uv(FUV);
